...
const UserBook = () => {
  ...
  const issueBook = async (details) => {
    try {
      const response = await axios.post(
        `http://${apiUrl}/issues/issueBook`,
        {
          userId: details.userId,
          bookModelId: details.bookModelId,
        },
        {
          headers: {
      ...
      setIsBookIssued(response.data);
          setIssuedBooks((prev) => [...prev, details]); 
  ...
  const handleIssueBook = (userId, bookModelId) => {
    setIssueDetails({ userId, bookModelId });
  };
